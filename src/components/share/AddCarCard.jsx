...
const AddCarCard = async ({ car }) => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_SERVER_API}/car/6a0c24fffa779d19ecdaa6c0`)
    // console.log(car, 'a car')
    console.log(car, 'my added car')
    return (
        <Card className='flex-row justify-between items-center rounded-md p-2 px-5 bg-white shadow   hover:shadow-2xl hover:scale-103 hover:-translate-y-2 duration-500'>
            <div className="grid md:flex itec gap-3">
                <Image src={car?.imageURL?.trim()} alt='booking car image' width={300} height={256} className='max-w-30 h-20 object-cover rounded-2xl' />
